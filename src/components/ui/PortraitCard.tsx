...
export const PortraitCard: React.FC<{ className?: string }> = ({ className = "" }) => {
  ...
  return (
    <motion.div
      ...
      className={`relative rounded-3xl p-1 bg-gradient-to-b from-customGreen/40 via-white/10 to-emerald-500/20 shadow-[0_20px_60px_rgba(0,0,0,0.8)] group transition-all duration-300 ${className}`}
    >
      {/* Outer Illuminated LED Glow Frame matching the photo */}
      <div className="absolute -inset-1 rounded-[28px] bg-gradient-to-b from-customGreen/30 via-emerald-500/10 to-customGreen/30 blur-lg opacity-70 group-hover:opacity-100 transition-opacity pointer-events-none" />
